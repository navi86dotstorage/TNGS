...
newBwa = "/home/nirtbact/MTBseq_source/opt/bwa"
newSamtools = "/home/nirtbact/MTBseq_source/opt/samtools"
######################################################
...
class Main:
    ...
    def recLength(self, inbam):
        ...
        '''
        output = {}
        process = subprocess.Popen([newSamtools, "view", "-H", inbam], stdout=subprocess.PIPE)
    ...
    def dealDeletion(self, inbam, min):
        ...
        '''
        process = subprocess.Popen([newSamtools, "view", inbam, "7bp_pks15.1:1-167"], stdout=subprocess.PIPE)
        deletion = []
        for line in iter(process.stdout.readline, ''):
            tmp = line.split('\t')
    ...
    def predLineage(self, inDict):
        inLineage = {
            "L1":"Mtb: Indo-Oceanic/East-African-Indian - Lineage 1 - (RD239)",
            "L21":"Mtb: East Asian - Lineage 2.1 - (RD105) non-Beijing",
            "L221":"Mtb: East Asian - Lineage 2.2.1 - (RD105,207,181) Beijing",
            "L2211":"Mtb: East Asian - Lineage 2.2.1.1 - (RD105,207,181,150) Beijing",
            "L2212":"Mtb: East Asian - Lineage 2.2.1.2 - (RD105,207,181,142) Beijing",
            "L222":"Mtb: East Asian - Lineage 2.2.2 - (RD105,207) Beijing",
            "L3":"Mtb: Central Asia Specific CAS - Lineage 3 - (RD750)",
...
if __name__ == "__main__":
    ...
    try:
        subprocess.call([newBwa, "index", dir+"/Reference/RDs30.fasta"])
    except OSError:
        print "Please check the installation of BWA."
        raise

    try:
        subprocess.call([newSamtools, "faidx", dir+"/Reference/RDs30.fasta"])
    except OSError:
    ...
    # BWA alignment
    if narg == 1:
        os.system(newBwa +" mem -R '@RG\\tID:RD\\tSM:RD\\tLB:RD\\tPL:Illumina' %s %s > %s" % ( dir+"/Reference/RDs30.fasta", input1, outprefix+".sam"))

    if narg == 2:
        os.system(newBwa +" mem -R '@RG\\tID:RD\\tSM:RD\\tLB:RD\\tPL:Illumina' %s %s %s > %s" % ( dir+"/Reference/RDs30.fasta", input1, input2, outprefix+".sam"))

    # Sam to Bam
    os.system(newSamtools +" view -bS -o %s %s" % (outprefix+".bam", outprefix+".sam"))

    # Sort Bam file
    os.system(newSamtools+" sort -o %s %s" % (outprefix+"sorted.bam", outprefix+".bam"))
    os.system(newSamtools+" index %s" % outprefix+"sorted.bam")

    # Calculate depth
    os.system(newSamtools+" depth %s > %s" % (outprefix+"sorted.bam", outprefix+".depth"))
    ...
    RD_length = t.recLength(outprefix+"sorted.bam")
    ...
    [deletionStat,deletionPred] = t.dealDeletion(outprefix+'sorted.bam', min)
    ...
    # Cleaning up
    if not debug:
        post = ['.sam','.bam','sorted.bam','sorted.bam.bai']
        for i in post:
